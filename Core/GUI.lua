...
local function CreateQualityOfLifeSettings(parentContainer)
    ...
    local ExperienceBarToggle = AG:Create("CheckBox")
    ExperienceBarToggle:SetLabel("Enable Experience Bar")
    ExperienceBarToggle:SetDescription("|cFFCCCCCCShows a custom Experience Bar.|r")
    ExperienceBarToggle:SetValue(QualityOfLifeDB.ExperienceBar)
    ExperienceBarToggle:SetCallback("OnValueChanged", function(_, _, value) QualityOfLifeDB.ExperienceBar = value FUI:UpdateExperienceBar() end)
    ExperienceBarToggle:SetRelativeWidth(1)
    TogglesContainer:AddChild(ExperienceBarToggle)

    local CombatTimerToggle = AG:Create("CheckBox")
    CombatTimerToggle:SetLabel("Enable Combat Timer")
    CombatTimerToggle:SetDescription("|cFFCCCCCCShows a combat timer while in combat.|r")
    CombatTimerToggle:SetValue(QualityOfLifeDB.CombatTimer)
    CombatTimerToggle:SetCallback("OnValueChanged", function(_, _, value) QualityOfLifeDB.CombatTimer = value FUI:UpdateCombatTimer() end)
    CombatTimerToggle:SetRelativeWidth(1)
    TogglesContainer:AddChild(CombatTimerToggle)

    local AutoQuestToggle = AG:Create("CheckBox")
    AutoQuestToggle:SetLabel("Enable Auto Quest")
    AutoQuestToggle:SetDescription("|cFFCCCCCCAutomatically accepts quests and auto-turns in quests with no reward choice.|r")
    AutoQuestToggle:SetValue(QualityOfLifeDB.AutoQuest)
    AutoQuestToggle:SetCallback("OnValueChanged", function(_, _, value) QualityOfLifeDB.AutoQuest = value FUI:UpdateAutoQuest() end)
    AutoQuestToggle:SetRelativeWidth(1)
    TogglesContainer:AddChild(AutoQuestToggle)
